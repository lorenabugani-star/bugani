...
(() => {
  ...
  /* ---------------- Carrossel simples (clientes / portfólio) ----------------
     Mostra sempre 2 linhas completas por página (esconde o resto com
     display:none) e pagina de verdade com as setas — em vez de depender do
     grid "vazar" horizontalmente, que não funciona porque o CSS Grid quebra
     linha sozinho quando tem mais itens do que colunas. */
  document.querySelectorAll("[data-carousel]").forEach((root) => {
    ...
    const perPage = () => columnsForWidth() * rowsPerPage;
    ...
    const update = () => {
      ...
      const pp = perPage();
      items.forEach((el, i) => {
        el.style.display = i >= page * pp && i < (page + 1) * pp ? "" : "none";
      });
