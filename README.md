...
O menu usa a arte oficial da logo (`assets/logo-white.png`, a versão branca,
pensada pro fundo preto do menu). O rodapé continua com o nome em texto,
porque essa versão da logo é branca e ficaria pouco legível no fundo
dourado do rodapé — se tiver uma versão escura/colorida da marca, me manda
que eu troco lá também. O favicon usa o ícone do leão (`assets/favicon.png`
e `assets/apple-touch-icon.png`).

A imagem de compartilhamento (og-image, a que aparece ao colar o link no
WhatsApp/Instagram/LinkedIn) ainda é um placeholder de texto preto/dourado.
Se quiser, me mande uma arte pronta pra isso (1200x630px) ou o arquivo
colorido da logo que eu monto:
- `assets/og-image.png` (imagem de compartilhamento — ainda placeholder)
...
O site já está configurado para `https://www.bugani.com.br` nas tags de
SEO (`canonical`, `og:*`, `twitter:*`), no `robots.txt` e no `sitemap.xml`.
Se o domínio final mudar, procure por `bugani.com.br` (Ctrl/Cmd+Shift+F) e
